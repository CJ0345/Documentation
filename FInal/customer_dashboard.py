...
class CustomerPage(ctk.CTk):
    def __init__(self):
        ...
        # Bind mouse enter and leave events to change colors
    ...
    def show_orders(self):
        ...
        for row in cursor.fetchall():
            tree.insert("", ctk.END, values=(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], f"₱{row[8]:.2f}"))
        ...
        def edit_order():
            ...
            contact_number = item_values[4]
            quantity = item_values[5]
            delivery_date = item_values[6]
            delivery_time = item_values[7]
            total_price = float(item_values[8].strip("₱"))

            edit_order_tab = ctk.CTkToplevel(self)
            edit_order_tab.title("Edit Order")
            edit_order_tab.geometry("500x700")
            ...
            ctk.CTkLabel(edit_order_tab, text="Delivery Time (HH:MM):").pack(pady=5)
